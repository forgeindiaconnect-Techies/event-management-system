...
function HelpMenu({ onSelect }) {
  ...
  const [launcher, setLauncher] = useState("navbar");
  ...
  return (
    <div className="fic-help-menu" ref={menuRef}>
      <button type="button" className="fic-help-trigger" aria-label="Open Help and Support" aria-expanded={open && launcher === "navbar"} title="Help and Support" onClick={() => {
        setLauncher("navbar");
        setView("menu");
        setOpen((value) => launcher === "navbar" ? !value : true);
      }}>
      ...
      <button
        type="button"
        className="fic-assistant-floating-trigger"
        aria-label="Open FIC AI Assistant"
        aria-expanded={open && launcher === "floating"}
        title="Ask FIC Assistant"
        onClick={() => {
          setLauncher("floating");
          setView("assistant");
          setOpen((value) => launcher === "floating" ? !value : true);
        }}
      >
        <span className="fic-assistant-floating-glow" aria-hidden="true" />
        <BsChatDots size={25} />
        <span className="fic-assistant-floating-dots" aria-hidden="true">
          <i />
          <i />
          <i />
        </span>
      ...
      {open && (
        <div className={`fic-help-popup ${launcher === "floating" ? "is-floating" : ""}`} role="dialog" aria-label="Help and Support">
          {view === "assistant" ? (
...
const styles = `
  ...
  .fic-help-popup{position:absolute;top:calc(100% + 14px);right:0;z-index:3001;width:340px;overflow:hidden;border:1px solid #e4e7ec;border-radius:14px;background:#fff;color:#172033;box-shadow:0 18px 45px rgba(15,23,42,.22)}
  .fic-help-popup.is-floating{position:fixed;top:auto;right:24px;bottom:96px}
  .fic-assistant-floating-trigger{position:fixed;right:24px;bottom:24px;z-index:3000;display:grid;width:58px;height:58px;place-items:center;padding:0;border:1px solid rgba(255,255,255,.7);border-radius:50%;background:linear-gradient(135deg,#6457ed,#392db5);color:#fff;box-shadow:0 0 0 0 rgba(99,84,235,.48),0 12px 30px rgba(46,35,151,.38);transition:transform .2s ease,box-shadow .2s ease;animation:ficAssistantPulse 2.4s ease-out infinite}
  .fic-assistant-floating-trigger:hover{transform:translateY(-3px) scale(1.06);box-shadow:0 0 24px rgba(116,101,255,.72),0 16px 34px rgba(46,35,151,.45)}
  .fic-assistant-floating-trigger:focus-visible{outline:3px solid rgba(99,84,235,.3);outline-offset:4px}
  .fic-assistant-floating-glow{position:absolute;inset:-7px;z-index:-1;border-radius:50%;background:radial-gradient(circle,rgba(126,111,255,.42),transparent 68%);animation:ficAssistantGlow 1.8s ease-in-out infinite alternate}
  .fic-assistant-floating-dots{position:absolute;right:7px;bottom:8px;display:flex;gap:2px;padding:3px 4px;border-radius:8px;background:#fff}
  .fic-assistant-floating-dots i{width:3px;height:3px;border-radius:50%;background:#5548d9;animation:ficAssistantDot 1.2s ease-in-out infinite}.fic-assistant-floating-dots i:nth-child(2){animation-delay:.15s}.fic-assistant-floating-dots i:nth-child(3){animation-delay:.3s}
  @keyframes ficAssistantPulse{0%,100%{box-shadow:0 0 0 0 rgba(99,84,235,.42),0 12px 30px rgba(46,35,151,.38)}55%{box-shadow:0 0 0 12px rgba(99,84,235,0),0 12px 30px rgba(46,35,151,.38)}}
  @keyframes ficAssistantGlow{from{opacity:.45;transform:scale(.94)}to{opacity:1;transform:scale(1.08)}}
  @keyframes ficAssistantDot{0%,60%,100%{transform:translateY(0)}30%{transform:translateY(-2px)}}
  ...
  @media(max-width:576px){.fic-help-popup{position:fixed;top:58px;right:12px;left:12px;width:auto}.fic-help-popup.is-floating{top:auto;right:12px;bottom:84px;left:12px;max-height:calc(100vh - 100px)}.fic-assistant-floating-trigger{right:16px;bottom:16px;width:54px;height:54px}}
  @media(prefers-reduced-motion:reduce){.fic-assistant-floating-trigger,.fic-assistant-floating-glow,.fic-assistant-floating-dots i{animation:none}}
`;
