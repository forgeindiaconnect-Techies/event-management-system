...
import { ActivityIndicator, SafeAreaView, ScrollView, StyleSheet, Text, View, TouchableOpacity, Modal, Alert } from 'react-native';
...
export default function AttendanceScreen() {
  ...
  const markAttendance = async (registrationId: number) => {
    try {
      const response = await fetch(`${API_BASE_URL}/registrations/${registrationId}/attendance`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${session?.token}` }
      });
      if (response.ok) {
        loadRegistrations(); // Refresh list after marking
      } else {
        ...
        {loading && registrations.length === 0 ? (
          ...
          <>
            <View style={styles.statsGrid}>
              ...
              <View style={styles.statCard}>
                <Ionicons name="person-outline" size={24} color="#b91c1c" />
                <Text style={styles.statLabel}>Absent</Text>
                <Text style={styles.statValue}>{absent}</Text>
              </View>
              <View style={styles.statCard}>
                <Ionicons name="stats-chart-outline" size={24} color="#0369a1" />
              ...
              registrations.map((reg, i) => (
                <View key={reg.id || i} style={styles.regCard}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
                    ...
                    {reg.attended ? (
                      <View style={[styles.actionBtn, { backgroundColor: '#15803d' }]}>
                        <Ionicons name="checkmark-done" size={16} color="#fff" />
                        <Text style={styles.actionBtnText}>Present</Text>
                      </View>
                    ) : (
                      <TouchableOpacity 
                        style={[styles.actionBtn, { backgroundColor: '#3d2e9c' }]}
                        onPress={() => markAttendance(reg.id)}
                      >
...
const styles = StyleSheet.create({
  ...
  regEmail: { fontSize: 14, color: '#68667a' },
  
  badge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
  badgeText: { fontSize: 11, fontWeight: '700', textTransform: 'uppercase' },

  attendanceBadge: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16 },
