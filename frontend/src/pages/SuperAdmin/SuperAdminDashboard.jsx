...
function SuperAdminDashboard({ section = "overview" }) {
  ...
  const deletePortal = async (portal) => {
    ...
    try {
      ...
      setPortals((current) => current.filter((item) => item.portalId !== portal.portalId));
      setDashboard((current) => ({
        ...current,
        totalPortals: Math.max(0, Number(current.totalPortals || 0) - 1),
        totalUsers: Math.max(0, Number(current.totalUsers || 0) - Number(portal.totalUsers || 0)),
        totalEvents: Math.max(0, Number(current.totalEvents || 0) - Number(portal.totalEvents || 0)),
        totalRegistrations: Math.max(0, Number(current.totalRegistrations || 0) - Number(portal.totalRegistrations || 0)),
        totalRevenue: Math.max(0, Number(current.totalRevenue || 0) - Number(portal.revenue || 0)),
      }));
      setMessage(`${portal.portalName} was deleted. Its users have been permanently logged out.`);
    } catch (error) {
      console.error(error);
...
function PageContent({ section, dashboard, portals, allPortals, money, search, setSearch, email, onDeletePortal, deletingPortalId, onPlatformChange }) {
  if (section === "portals") return <PortalTable portals={portals} money={money} search={search} setSearch={setSearch} onDeletePortal={onDeletePortal} deletingPortalId={deletingPortalId} />;

  if (section === "revenue") return <>
    <div className="sa-page-cards three"><Metric icon={<BsCashCoin />} label="Total Revenue" value={money(dashboard.totalRevenue)} /><Metric icon={<BsBarChart />} label="Monthly Revenue" value={money(dashboard.monthlyRevenue)} /><Metric icon={<BsBuilding />} label="Revenue Portals" value={dashboard.activePortals} /></div>
    <Panel title="Revenue by portal" subtitle="Revenue generated by each company portal."><PortalTable portals={portals} money={money} compact /></Panel>
  </>;
  ...
  if (section === "users") return <>
    ...
    <Panel title="Users by portal" subtitle="User totals for every portal."><SimplePortalList portals={allPortals} field="totalUsers" label="users" /></Panel>
  </>;

  if (section === "reports") return <div className="sa-page-cards four"><Metric icon={<BsBuilding />} label="Portals" value={dashboard.totalPortals} /><Metric icon={<BsPeople />} label="Users" value={dashboard.totalUsers} /><Metric icon={<BsCalendarEvent />} label="Events" value={dashboard.totalEvents} /><Metric icon={<BsCheckCircle />} label="Registrations" value={dashboard.totalRegistrations} /></div>;
  ...
  return <div className="sa-innovative-overview">
    ...
    <section className="sa-flow-card">
      ...
      <div className="sa-flow-total"><span>Total revenue</span><strong>{money(dashboard.totalRevenue)}</strong><small>Across all company portals</small></div>
      <div className="sa-flow-track"><i /><i /><i /><i /><i /></div>
      <div className="sa-flow-stats"><div><span>This month</span><strong>{money(dashboard.monthlyRevenue)}</strong></div><div><span>Portal health</span><strong>{activeRate}%</strong></div></div>
    </section>
    ...
    <section className="sa-activity-card">
      ...
      <div className="sa-activity-body">
        ...
        <div className="sa-activity-metrics">
          <div className="sa-activity-stat"><span>Events created</span><strong>{dashboard.totalEvents || 0}</strong><small>Across all portals</small></div>
          <div className="sa-activity-stat"><span>Registrations</span><strong>{dashboard.totalRegistrations || 0}</strong><small>Total participants</small></div>
        </div>
      </div>
    </section>
  </div>;
...
function EmailDeliveryMonitor() {
  ...
  return (
    <div className="sa-subscriptions-page">
      <div className="sa-page-cards four">
      ...
      <Panel title="Email delivery history" subtitle="Inspect SMTP delivery status and retry failed messages.">
        ...
        <div className="table-responsive">
          <table className="table align-middle sa-native-table">
            ...
            <tbody>
              ...
              {!loading && deliveries.map((delivery) => (
                <tr key={delivery.id}>
                  ...
                  <td style={{ maxWidth: 220 }}><small className="text-danger">{delivery.failureReason || "—"}</small></td>
                  <td>
...
function SuperAdminSubscriptions({ money, onPlatformChange }) {
  ...
  const updatePlan = async (form) => {
    try {
      ...
      setEditingPlan(null);
      setNotice({ type: "success", text: form.displayName + " was updated successfully." });
      await loadData();
    } catch (error) {
  ...
  const changeCurrentPlan = async (item) => {
    const planCode = window.prompt(
      "Enter the replacement plan: STANDARD, PROFESSIONAL, or ENTERPRISE",
      item.planCode === "STANDARD" ? "PROFESSIONAL" : "STANDARD"
    );
    if (!planCode?.trim()) return;
    const normalizedPlan = planCode.trim().toUpperCase();
    if (!["STANDARD", "PROFESSIONAL", "ENTERPRISE"].includes(normalizedPlan)) {
      setNotice({ type: "error", text: "Choose STANDARD, PROFESSIONAL, or ENTERPRISE." });
      return;
    }
  ...
  return (
    <div className="sa-sub-modal-backdrop" onMouseDown={onClose}>
      <div className="sa-plan-editor" onMouseDown={(event) => event.stopPropagation()}>
        <header><div><span>Plan configuration</span><h2>Edit {plan.displayName}</h2></div><button onClick={onClose}><BsXLg /></button></header>
        ...
        <footer><button onClick={onClose}>Cancel</button><button className="primary" onClick={() => onSave(form)} disabled={saving}>{saving ? "Saving..." : "Save plan"}</button></footer>
      </div>
    </div>
  );
}
...
function Metric({ icon, label, value }) { return <div className="admin-bento-card sa-metric"><span className="admin-bento-icon">{icon}</span><p>{label}</p><h2>{value ?? 0}</h2></div>; }
...
function PortalTable({ portals, money, search, setSearch, compact = false, onDeletePortal, deletingPortalId }) {
  const [selectedPortal, setSelectedPortal] = useState(null);
  return <div className={compact ? "sa-embedded-table" : "admin-bento-card sa-portal-card"}>
    {!compact && <div className="sa-table-heading"><div><h2>Portal Companies</h2><p>Companies and owners using the platform.</p></div><label><BsSearch /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search portals..." /></label></div>}
    <div className="table-responsive"><table className="sa-native-table"><thead><tr><th>Portal</th><th>Owner</th><th>Plan</th><th>Status</th><th>Events</th><th>Users</th><th>Registrations</th><th>Revenue</th>{!compact && <th>Actions</th>}</tr></thead><tbody>
      {portals.map((portal) => <tr key={portal.portalId} onClick={() => !compact && setSelectedPortal(portal)} style={{ cursor: compact ? "default" : "pointer" }}><td><strong>{portal.portalName || "Unnamed portal"}</strong><small>{portal.portalCode || "No code"}</small></td><td><strong>{portal.ownerName || "Not assigned"}</strong><small>{portal.ownerEmail || "No email"}</small></td><td>{portal.plan || "Free"}</td><td><span className={`status ${String(portal.status || "inactive").toLowerCase()}`}>{portal.status || "Inactive"}</span></td><td>{portal.totalEvents || 0}</td><td>{portal.totalUsers || 0}</td><td>{portal.totalRegistrations || 0}</td><td><strong>{money(portal.revenue)}</strong></td>{!compact && <td><button type="button" className="sa-delete-portal" onClick={(event) => { event.stopPropagation(); onDeletePortal(portal); }} disabled={deletingPortalId === portal.portalId}><BsTrash />{deletingPortalId === portal.portalId ? "Deleting..." : "Delete"}</button></td>}</tr>)}
      {!portals.length && <tr><td colSpan={compact ? "8" : "9"}><Empty text="No portals found." /></td></tr>}
    </tbody></table></div>
    {selectedPortal && <div className="sa-modal-backdrop" onClick={() => setSelectedPortal(null)}><div className="sa-action-modal" onClick={(event) => event.stopPropagation()}><button type="button" className="sa-modal-close" onClick={() => setSelectedPortal(null)}><BsXLg /></button><span className="subscription-eyebrow">Portal details</span><h2>{selectedPortal.portalName || "Unnamed portal"}</h2><div className="sa-admin-receipt-meta"><span>Portal code</span><strong>{selectedPortal.portalCode || "Not available"}</strong><span>Owner</span><strong>{selectedPortal.ownerName || "Not assigned"}</strong><span>Email</span><strong>{selectedPortal.ownerEmail || "Not available"}</strong><span>Phone</span><strong>{selectedPortal.ownerPhone || "Not available"}</strong><span>Current plan</span><strong>{selectedPortal.plan || "No plan"}</strong><span>Status</span><strong>{selectedPortal.status || "Inactive"}</strong><span>Events</span><strong>{selectedPortal.totalEvents || 0}</strong><span>Users</span><strong>{selectedPortal.totalUsers || 0}</strong><span>Registrations</span><strong>{selectedPortal.totalRegistrations || 0}</strong><span>Revenue</span><strong>{money(selectedPortal.revenue)}</strong></div></div></div>}
  </div>;
}
