...
export default function LandingScreen() {
  ...
  const BANNERS = [
    ...
    },
    {
      ...
      image: 'https://images.unsplash.com/photo-1540039155732-684735035727?q=80&w=1000&auto=format&fit=crop',
    },
    {
      id: 3,
      tag: 'TECH SUMMIT',
  ...
  if (isSearchActive) {
    filteredEvents = filteredEvents.filter(e => eventMatchesCategory(e, currentCategory));
    if (searchQuery.trim()) {
  ...
  const trending = [...futureEvents].sort((a, b) => b.createdAt - a.createdAt).slice(0, 10);
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          ...
          <TouchableOpacity style={[styles.notificationBtn, { backgroundColor: colors.surface }]} onPress={() => console.log('Call Notification API')}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                    ...
                    >
                      <View style={styles.bannerOverlay}>
                        <View style={styles.dotGrid}>
                          <View style={styles.dotRow}><View style={styles.smallDot}/><View style={styles.smallDot}/><View style={styles.smallDot}/></View>
                          <View style={styles.dotRow}><View style={styles.smallDot}/><View style={styles.smallDot}/><View style={styles.smallDot}/></View>
                          <View style={styles.dotRow}><View style={styles.smallDot}/><View style={styles.smallDot}/><View style={styles.smallDot}/></View>
                        </View>
                        <View style={styles.bannerContent}>
                          ...
                          <View style={styles.dashedRow}>
                            ...
                            <Ionicons name="star" size={12} color="#FFB900" style={{ marginHorizontal: 4 }} />
                            <View style={styles.dashLine} />
                          </View>
                          <Text style={styles.bannerSubtitle}>{banner.title}</Text>
                          <TouchableOpacity 
                            style={styles.bookNowBtnSmall}
                            onPress={() => router.push({ pathname: '/event-detail', params: { id: banner.id } })}
                          >
                ...
                <View key={banner.id} style={styles.bannerContainer}>
                  ...
                  >
                    <View style={styles.bannerOverlayDefault}>
                      ...
                      <Text style={styles.bannerTitleDefault}>{banner.title}</Text>
                      <TouchableOpacity 
                        style={styles.bookNowBtnSmallDefault}
                        onPress={() => router.push({ pathname: '/event-detail', params: { id: banner.id } })}
                      >
                        <Text style={styles.bookNowTextSmallDefault}>Book Now</Text>
                      </TouchableOpacity>
                    </View>
                  </ImageBackground>
                </View>
        ...
        {loading ? (
          <ActivityIndicator size="large" color="#7931ED" style={{ marginTop: 40 }} />
        ) : (
          <>
            ...
            {isSearchActive && searchResults.length === 0 && (
              <View style={{ alignItems: 'center', marginTop: 40, marginBottom: 20 }}>
                ...
                <Text style={{ fontSize: 18, fontWeight: '700', color: colors.text, marginTop: 12 }}>No events found</Text>
                <Text style={{ fontSize: 14, color: colors.textMuted, marginTop: 4 }}>Try adjusting your search query.</Text>
              </View>
            )}

            {/* Trending Events */}
            <View style={[styles.sectionHeader, { marginTop: isSearchActive ? 24 : 0 }]}>
            ...
            >
              {trending.map(event => (
                <TouchableOpacity key={event.id} style={styles.trendingCard} onPress={() => router.push({ pathname: '/event-detail', params: { id: event.id } })}>
                  <ImageBackground 
                    source={{ uri: event.image || 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?q=80&w=1000&auto=format&fit=crop' }} 
                    style={styles.trendingImage}
                    imageStyle={{ borderRadius: 20 }}
            ...
            {upcoming.length > 0 && (
              <>
                ...
                >
                  {upcoming.map(event => (
                    <TouchableOpacity key={event.id} style={styles.upcomingSquareCard} onPress={() => router.push({ pathname: '/event-detail', params: { id: event.id } })}>
                      <ImageBackground 
                        source={{ uri: event.image || 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?q=80&w=1000&auto=format&fit=crop' }} 
                      ...
                      >
                        <View style={styles.upcomingSquareOverlay}>
                          ...
                          <View style={styles.upcomingSquareFooter}>
                            <Text style={styles.upcomingSquareTitle} numberOfLines={2}>{event.title}</Text>
                            <Ionicons name="arrow-forward-outline" size={16} color="#FFF" style={{ marginLeft: 8 }} />
                          </View>
                        </View>
                      </ImageBackground>
                    </TouchableOpacity>
